      ...
      child: Row(
        children: [
          ...
          TextButton(
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Logged a call to ${item.customer.fullName}.')),
              );
            },
...
class _TreatmentRecordDialogState extends State<_TreatmentRecordDialog> {
  ...
  void _save() {
    ...
    final next = store.complete(
      appt.id,
      productsUsed: _products.toList(),
      notes: _notes.text.trim(),
      photoCount: _consent ? _photoCount : 0,
    ...
    if (customerId.isEmpty) {
      ...
      final created = store.addCustomer(
      ...
      actions: [
        ...
        FilledButton(
          onPressed: () {
            final ok = context
                .read<StaffStore>()
                .reschedule(a.id, _date, _time, capacity: kBranchCapacity);
            if (!ok) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
      ...
      actions: [
        ...
        FilledButton(
          style: FilledButton.styleFrom(
              backgroundColor: Theme.of(context).colorScheme.error),
          onPressed: () {
            context.read<StaffStore>().cancel(
